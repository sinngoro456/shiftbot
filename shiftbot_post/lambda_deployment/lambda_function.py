...
def lambda_handler(event, context):
    ...
    def handle_message(event):
        ...
        if send_message == "今日のドリンクメニュー" and isinstance(
            event.source, SourceUser
        ):
            profile = line_bot_api.get_profile(event.source.user_id)
            ...
            message = FlexSendMessage(
                alt_text="お問い合わせ", contents=json.loads(bubble_string)
            )
    ...
    def handle_postback(event):
        if event.postback.data == "action=question&id=1":
            line_bot_api.reply_message(
                ...
                (
                    TextSendMessage(
                        text="平日は8:30~17:00、祝休日は8:30~21:00まで開いています。"
                    )
                ),
            )
            ...
            line_bot_api.reply_message(
                ...
                (
                    TextSendMessage(
                        text="はい、車6台が停めれる駐車場がございます。また、駐輪場もございます。"
                    )
                ),
            )
            ...
            line_bot_api.reply_message(
                ...
                (
                    TextSendMessage(
                        text="申し訳ございません。デリバリーもテイクアウトも今はやっておりません"
                    )
                ),
            )
